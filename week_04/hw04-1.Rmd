---
title: "Regression of Instacart Market Basket Analysis"
author: "Claire Liu"
...
```{r}
fit_m2 <- data.frame(data[, c(6, 2, 9, 10, 11)], 
                     fitted = fitted(m2), 
                     resid = resid(m2), 
...
```{r}
ggplot(data = fit_m2, aes(x = 地科, group = 性別)) +
  stat_density(geom = 'path', position = 'identity') +
  stat_density(geom = 'path', position = 'identity', aes(x = fitted)) +
  geom_vline(xintercept = c(with(data, tapply(地科,性別, mean))), linetype = 'dotted')+
  facet_grid(性別 ~ .) +
...
##### 看看不同國家、有無電腦的學生地科平均與對應的平均數標準誤

```{r}
summary(m4 <- lm(地科 ~ 國家 + 電腦, data = data[, c('電腦', '地科', '國家') ]))
coefplot(m4, predictors = c('電腦', '地科', '國家'),
...
* [Mac 的 TextEdit 文件如何存成 UTF-8 的 txt 檔](https://medium.com/彼得潘的-swift-ios-app-開發問題解答集/mac-的-textedit-文件如何存成-txt-檔-5f647c1582ac)
